...
class PipelineRunner:
    ...
    def _execute_task(self, task: dict) -> dict:
        ...
        image_generator_module = self._load_module("generator.image_generator") if needs_generate else None
        image_upscaler_module = self._load_module("upscaler.image_upscaler") if needs_upscale else None
        deviantart_upload_module = self._load_module("uploader.deviantart_upload") if needs_upload else None
        ...
        if needs_generate:
            if image_generator_module is None:
                raise RuntimeError("Image generator module unavailable")
        ...
        if needs_upscale:
            if image_upscaler_module is None:
                raise RuntimeError("Image upscaler module unavailable")
            if not image_path:
        ...
        if needs_upload:
            if deviantart_upload_module is None:
                raise RuntimeError("Uploader module unavailable")
    ...
    def run_once(self, mode: str = "full") -> dict:
        image_generator_module = self._load_module("generator.image_generator")
        ...
        image_upscaler_module = self._load_module("upscaler.image_upscaler")
        upscaled_path = self._retry("image upscale", image_upscaler_module.upscale_image, image_path)
        record_id = save_image_record(
            prompt=prompt,
            image_path=image_path,
        ...
        deviantart_upload_module = self._load_module("uploader.deviantart_upload")
        upload_payload = self._retry("image upload", deviantart_upload_module.upload_image, upscaled_path, prompt)
        mark_uploaded(record_id, uploaded=True, upscaled_path=upscaled_path)
        result = {
            "id": record_id,
            "prompt": prompt,
